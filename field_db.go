...
func fieldToStringTCPFlags(data []byte) (flags string) {
	if data[0] & 0x80 > 0 { flags += "C" } else { flags += " " }
	if data[0] & 0x40 > 0 { flags += "E" } else { flags += " " }
	if data[0] & 0x20 > 0 { flags += "U" } else { flags += " " }
	if data[0] & 0x10 > 0 { flags += "A" } else { flags += " " }
	if data[0] & 0x08 > 0 { flags += "P" } else { flags += " " }
	if data[0] & 0x04 > 0 { flags += "R" } else { flags += " " }
	if data[0] & 0x02 > 0 { flags += "S" } else { flags += " " }
	if data[0] & 0x01 > 0 { flags += "F" } else { flags += " " }
	return
}
...
func fieldToStringSamplingAlgo(data []byte) string {
	switch data[0] {
	case 0x01: return "Deterministic"
	case 0x02: return "Random"
...
func fieldToStringEngineType(data []byte) string {
	switch data[0] {
	case 0x00: return "Routing Processor"
	case 0x01: return "Linecart"
	default: return "Unknown"
	}
}
...
func fieldToStringMPLSTopLabelType(data []byte) string {
	switch data[0] {
	case 0x01: return "TE-MIDPT"
	case 0x02: return "ATOM"
	case 0x03: return "VPN"
	case 0x04: return "BGP"
	case 0x05: return "LDP"
	default: return "UNKNOWN"
	}
}
