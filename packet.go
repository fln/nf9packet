...
func (this *Packet) DataFlowSets() (list []DataFlowSet) {
	for i, _ := range this.FlowSets {
		switch set := this.FlowSets[i].(type) {
		case DataFlowSet:
...
func (this *Packet) TemplateRecords() (list []*TemplateRecord) {
	for i, _ := range this.FlowSets {
		switch set := this.FlowSets[i].(type) {
		case TemplateFlowSet:
			for j, _ := range set.Records {
...
func (this *Packet) OptionsTemplateRecords() (list []*OptionsTemplateRecord) {
	for i, _ := range this.FlowSets {
		switch set := this.FlowSets[i].(type) {
